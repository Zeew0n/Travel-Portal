...
{
    public class HotelOfflineIssueTicketRepository
    {
        int Sno = 0;
        TravelPortalEntity.EntityModel _ent = new TravelPortalEntity.EntityModel();

        public List<HotelBookingRecordModel> GetOfflineHotelIssueTicketList()
        {
            var result = _ent.Htl_BookingRecord.Where(x => x.TicketStatusId == 29 || x.TicketStatusId == 16).OrderByDescending(x => x.CreatedOn);
        ...
        public IPagedList<HotelBookingRecordModel> GetPagedList(int? page)
        {
            ...
            return GetOfflineHotelIssueTicketList().ToPagedList(currentPageIndex, HotelGeneralRepository.DefaultPageSize);
        }
