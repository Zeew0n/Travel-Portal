...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="rptSearchResult">
        ...
        <table cellspacing="0" cellpadding="0" border="0" style="border-collapse: collapse;"
            ...
            <tbody>
                ...
                <tr id="tr_<%=sno %>" class="<%:classTblRow %>" onmouseover="this.className='GridRowOver'"
                    ...
                    <td>
                        <p>
                            <%:Html.ActionLink("Detail", "Detail", new { id = item.BookingRecordId, controller = "HotelBookingRecord", area = "Hotel" }, new { @class = "", @title = "Details" })%>
                            |
                          <%--   <%if(item.TicketStatusId!=13) %>
                             <%:Html.ActionLink("Cancel", "Cancel", new { id = item.BookingRecordId, controller = "HotelBookingRecord", area = "Hotel" }, new { @class = "", @title = "Cancel" })%>
                             |--%>
        ...
        <%--<%  if (Model.TabularList.TotalItemCount > ATLTravelPortal.Areas.Hotel.Repository.HotelGeneralRepository.DefaultPageSize)
            {%>--%>
        <div class="pager">
            <%:MvcHtmlString.Create(ATLTravelPortal.Areas.Hotel.Pagination.PagingExtensions.Pager(Html, ATLTravelPortal.Areas.Hotel.Repository.HotelGeneralRepository.DefaultPageSize, Model.TabularList.PageNumber, Model.TabularList.TotalItemCount,true,true,"", Url.Content("~/Hotel/HotelBookingRecord")))%>
        </div>
        <%  }
