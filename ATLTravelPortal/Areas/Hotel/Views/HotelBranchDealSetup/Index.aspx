...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <div class="pageTitle">
        <ul class="buttons-panel">
            ...
            <li>
                <%:Html.ActionLink("New Deal", "Create", "HotelMasterBranchDealSetUp", new { area="Hotel"}, new { @class = "createDeal linkButton", @title = "New Deal" })%>
            </li>
    ...
    <script type="text/javascript">
        ...
        $('#DeleteMasterDeal').live("click", function (event) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Hotel/HotelMasterBranchDealSetup/Delete/",
        ...
        function DeleteMasterDealForceFully(masterDealId, text) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Hotel/AjaxRequest/DeleteBranchMasterDealForceFully/",
                    data: { id: masterDealId, name: text },
                    error: function () {
                        $("#loadingIndicator").html('');
                        alert('Can not delete the deal ' + text);
        ...
        function EditDeal(divId, dealId) {
            ...
            $('#' + divId).load("/Hotel/AjaxRequest/AjaxBranchDealDetail/" + dealId + "?source=" + source);
        }

        function CancelDeal(divId, dealId) {
            $("#" + divId + "_loading").html('<center><img src="<%=Url.Content("~/Content/images/indicator.gif") %>" alt="" width="16px" height="16px" /></center>');
            $("#" + divId).load("/Hotel/AjaxRequest/AjaxBranchDealCancel/" + dealId);
        }
        ...
        function DeleteDeal(divId, dealId) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Hotel/HotelBranchDealSetup/Delete/",
                    data: { id: dealId },
                    error: function () {
                        $("#" + divId + "_loading").html('');
        ...
        $('#saveDeal').live("click", function (event) {
            ...
            $.ajax({
                type: "POST",
                url: '/Hotel/HotelBranchDealSetup/Create/',
        ...
        function UpdateDeal(divId, formId) {
            ...
            $.ajax({
                type: "POST",
                url: '/Hotel/HotelBranchDealSetup/Edit/',
                data: serializeFormData,
                dataType: "html",
                success: function (serverResponse) {
