<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/BranchOfficeMain.Master" Inherits="System.Web.Mvc.ViewPage<ATLTravelPortal.Areas.Airline.Models.BranchDealViewModel>" %>
    ...
    <script type="text/javascript">
        ...
        $('#DeleteMasterDeal').live("click", function (event) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Airline/MasterBranchDealSetup/Delete/",
        ...
        function DeleteMasterDealForceFully(masterDealId, text) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Airline/AjaxRequest/DeleteBranchMasterDealForceFully/",
                    data: { id: masterDealId, name: text },
                    error: function () {
                        $("#loadingIndicator").html('');
                        alert('Can not delete the deal ' + text);
        ...
        function EditDeal(divId, dealId) {
            ...
            $('#' + divId).load("/Airline/AjaxRequest/AjaxBranchDealDetail/" + dealId + "?source=" + source);
        }

        function CancelDeal(divId, dealId) {
            $("#" + divId + "_loading").html('<center><img src="<%=Url.Content("~/Content/images/indicator.gif") %>" alt="" width="16px" height="16px" /></center>');
            $("#" + divId).load("/Airline/AjaxRequest/AjaxBranchDealCancel/" + dealId);
        }
        ...
        function DeleteDeal(divId, dealId) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Airline/BranchDealSetup/Delete/",
                    data: { id: dealId },
                    error: function () {
                        $("#" + divId + "_loading").html('');
        ...
        $('#saveDeal').live("click", function (event) {
            ...
            $.ajax({
                type: "POST",
                url: '/Airline/BranchDealSetup/Create/',
        ...
        function UpdateDeal(divId, formId) {
            ...
            $.ajax({
                type: "POST",
                url: '/Airline/BranchDealSetup/Edit/',
                data: serializeFormData,
                dataType: "html",
                success: function (serverResponse) {
