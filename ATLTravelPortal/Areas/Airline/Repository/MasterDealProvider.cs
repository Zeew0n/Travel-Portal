...
namespace ATLTravelPortal.Areas.Airline.Repository
{
    public class MasterDealProvider
    {
        ...
        public IEnumerable<SelectListItem> GetAllDealMasterForDistributorAgentClassList(int productId, int distributorId)
        {
            List<Core_DistributorDealMasters> all = GetAllDealMasterForDistributorAgentClass(productId, distributorId).OrderBy(z => z.DistributorDealName).ToList();
            var GetAllDealMasterList = new List<SelectListItem>();
            foreach (var item in all)
            {
                var teml = new SelectListItem
        ...
        public IQueryable<Core_DistributorDealMasters> GetAllDealMasterForDistributorAgentClass(int productId, int distributorID)
        {
            return ent.Core_DistributorDealMasters.Where(x => x.ProductId == productId && x.DistributorId == distributorID).OrderBy(xx => xx.DistributorDealName).AsQueryable();
        }
        ...
        public void CopyBusDealfromOneToAnother(string newDealName, int copyFromDealId, int dealTypeId, DateTime? effectiveFrom, DateTime? expireOn)
        {
            ent.CopyBusDeal(newDealName, copyFromDealId, dealTypeId, effectiveFrom, expireOn);

        }

        public void CopyMobileDealfromOneToAnother(string newDealName, int copyFromDealId, int dealTypeId, DateTime? effectiveFrom, DateTime? expireOn)
        {
            ent.CopyMobileDeal(newDealName, copyFromDealId, dealTypeId, effectiveFrom, expireOn);
        }
        ...
        public IEnumerable<SelectListItem> GetBusDealIdentifiers(int id)
        {
            List<ServiceProviders> all = GetAllServiceProviders().Where(xx => xx.isActive == true && xx.ServiceType == "BUS").OrderBy(z => z.AgentDealIdentifier).ToList();
            var serviceProviderList = new List<SelectListItem>();
        ...
        public IEnumerable<SelectListItem> GetMobileDealIdentifiers(int id)
        {
            List<ServiceProviders> all = GetAllServiceProviders().Where(xx => xx.isActive == true && xx.ServiceType == "MOB").OrderBy(z => z.AgentDealIdentifier).ToList();
            var serviceProviderList = new List<SelectListItem>();
        ...
        public IEnumerable<SelectListItem> GetMobileServiceProviders()
        {
            List<ServiceProviders> all = GetAllServiceProviders().Where(xx => xx.isActive == true && xx.ServiceType == "MOB").OrderBy(z => z.AgentDealIdentifier).ToList();
            var serviceProviderList = new List<SelectListItem>();
        ...
        public int Save_Bus_Deals(DealViewModel model)
        {
            Bus_Deals objToSave = new Bus_Deals();
            ...
            objToSave.FromCityId = model.FromCityId;
            objToSave.ToCityId = model.ToCityId;
            objToSave.Markup = model.AdultMarkup;
            objToSave.isMarkupPercentage = model.isMarkupPercentage;
            objToSave.Commission = model.AdultBFCommission;

            objToSave.isCommissionPercentage = model.isBFCommissionPercentage;

            objToSave.VersionNo = 1;

            objToSave.CreatedBy = model.MakerId;
            objToSave.CreatedDate = model.MakerDate;

            ent.AddToBus_Deals(objToSave);
            ent.SaveChanges();
            return objToSave.BusDealId;
        }

        public int Save_Mobile_Deals(DealViewModel model)
        {
            MRC_Deals objToSave = new MRC_Deals();
            ...
            objToSave.isPercentage = model.isBFCommissionPercentage;
        ...
        public bool Update_Bus_Deals(DealViewModel model)
        {
            Edit_Tkt_Deals(model.DealId, model.UpdatedBy.Value);

            Bus_Deals objToUpdate = new Bus_Deals();
            objToUpdate = ent.Bus_Deals.Where(x => x.BusDealId == model.DealId).FirstOrDefault();
            ...
            objToUpdate.FromCityId = model.FromCityId;
            objToUpdate.ToCityId = model.ToCityId;
            objToUpdate.Markup = model.AdultMarkup;
            objToUpdate.isMarkupPercentage = model.isMarkupPercentage;
            objToUpdate.Commission = model.AdultBFCommission;
            objToUpdate.isCommissionPercentage = model.isBFCommissionPercentage;
        ...
        public bool Update_Mobile_Deals(DealViewModel model)
        {
            Edit_MRC_Deals(model.DealId, model.UpdatedBy.Value);

            MRC_Deals objToUpdate = new MRC_Deals();
            objToUpdate = ent.MRC_Deals.Where(x => x.DealId == model.DealId).FirstOrDefault();
            ...
            objToUpdate.Commission = model.AdultBFCommission;
            objToUpdate.isPercentage = model.isBFCommissionPercentage;
        ...
        public bool Delete_Bus_Deals(int dealId, int userId)
        {
            ent.Tkt_SaveBusChangesOnDeal(dealId, userId, "D");
            return true;
        }
        public bool Delete_Mobile_Deals(int dealId, int userId)
        {
            ent.Tkt_SaveMobileChangesOnDeal(dealId, userId, "D");
            return true;
        }
        ...
        public bool Delete_Tkt_BusDealMasters(int masterDealId, int userId)
        {
            ent.TKT_BusSaveChangesOnMasterDeal(masterDealId, userId, "D", false);
            return true;
        }

        public bool Delete_Tkt_MobileDealMasters(int masterDealId, int userId)
        {
            ent.Tkt_MobileSaveChangesOnMasterDeal(masterDealId, userId, "D", false);
            return true;
        }
        ...
        public bool Delete_TKT_BusForceDeleteMasterDeal(int masterDealId, int userId)
        {
            ent.TKT_BusSaveChangesOnMasterDeal(masterDealId, userId, "D", true);
            return true;
        }

        public bool Delete_MRC_MobileForceDeleteMasterDeal(int masterDealId, int userId)
        {
            ent.Tkt_MobileSaveChangesOnMasterDeal(masterDealId, userId, "D", true);
            return true;
        }
        ...
        public bool Edit_MRC_Deals(int dealId, int userId)
        {
            ent.Tkt_SaveMobileChangesOnDeal(dealId, userId, "E");
            return true;
        }
        ...
        public IEnumerable<DealViewModel> GetAllBusDeals(int? dealMasterId, string FilterDealIdentifierId, int? FilterOpratorId, int? FilterCategoryId, int? FilterCurrencyId)
        {

            List<DealViewModel> dealViewList = new List<DealViewModel>();

            IEnumerable<Bus_Deals> deals = new List<Bus_Deals>();
            if (dealMasterId != null)
                deals = ent.Bus_Deals.Where(xx => xx.DealMasterId == dealMasterId);

            if (!string.IsNullOrEmpty(FilterDealIdentifierId))
                deals = deals.Where(x => x.DealIdentifier == FilterDealIdentifierId);

            if (FilterOpratorId != null)
                deals = deals.Where(x => x.BusMasterId == FilterOpratorId);


            if (FilterCategoryId != null)
                deals = deals.Where(x => x.BusCategoryId == FilterCategoryId);

            if (FilterCurrencyId != null)
                deals = deals.Where(x => x.CurrencyId == FilterCurrencyId);

            foreach (Bus_Deals deal in deals)
            {
                DealViewModel dealViewModel = new DealViewModel();

                dealViewModel.DealId = deal.BusDealId;
                dealViewModel.DealMasterId = deal.DealMasterId;
                ...
                dealViewModel.DealIdentifierId = deal.BusDealId;
                dealViewModel.DealIdentifierText = deal.DealIdentifier;
                dealViewModel.SectorType = deal.SectorType;
                ...
                if (deal.FromCityId != null)
                    dealViewModel.FromCity = cityProvider.GetBusCityByid(deal.FromCityId.Value).BusCityName;
                dealViewModel.ToCityId = deal.ToCityId;
                if (deal.ToCityId != null)
                    dealViewModel.ToCity = cityProvider.GetBusCityByid(deal.ToCityId.Value).BusCityName;

                dealViewModel.AdultMarkup = deal.Markup;
                dealViewModel.isMarkupPercentage = deal.isMarkupPercentage;


                dealViewModel.AdultBFCommission = deal.Commission;
                dealViewModel.isBFCommissionPercentage = deal.isCommissionPercentage;

                dealViewModel.MakerId = deal.CreatedBy;
                dealViewModel.MakerDate = deal.CreatedDate;
            ...
            return dealViewList.OrderBy(x => x.BusOperatorName).ThenBy(x => x.Currency);
        }

        public IEnumerable<DealViewModel> GetAllMobileDeals(int? dealMasterId, string FilterDealIdentifierId)
        {
            List<DealViewModel> dealViewList = new List<DealViewModel>();

            IEnumerable<MRC_Deals> deals = new List<MRC_Deals>();

            if (dealMasterId != null)
                deals = ent.MRC_Deals.Where(xx => xx.DealMasterId == dealMasterId);
            ...
            foreach (MRC_Deals deal in deals)
            {
                DealViewModel dealViewModel = new DealViewModel();
                ...
                dealViewModel.AdultBFCommission = deal.Commission;
                dealViewModel.isBFCommissionPercentage = deal.isPercentage;
        ...
        public DealViewModel GetBusDealDetail(int? dealId)
        {
            List<DealViewModel> dealViewList = new List<DealViewModel>();

            List<Bus_Deals> deals = ent.Bus_Deals.Where(xx => xx.BusDealId == dealId).ToList();
            DealViewModel dealViewModel = new DealViewModel();
            foreach (Bus_Deals deal in deals)
            {
                dealViewModel.DealId = deal.BusDealId;
        ...
        public DealViewModel GetMobileDealDetail(int? dealId)
        {
            List<DealViewModel> dealViewList = new List<DealViewModel>();

            List<MRC_Deals> deals = ent.MRC_Deals.Where(xx => xx.DealId == dealId).ToList();
            DealViewModel dealViewModel = new DealViewModel();
            foreach (MRC_Deals deal in deals)
            {
                dealViewModel.DealId = deal.DealId;
