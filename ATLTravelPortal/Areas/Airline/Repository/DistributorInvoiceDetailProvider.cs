...
{
    public class DistributorInvoiceDetailProvider
    {
        private int createdBy { get; set; }
        private DateTime createdDate { get; set; }

        EntityModel _ent = new EntityModel();
        public DistributorInvoiceDetailProvider()
        { }
        ...
        {
            var masterPNRS = _ent.TBO_MasterPNRs.Where(x => x.AgentId == agentId);
            var filterdPNRS = masterPNRS.Where(x => x.MPNRId == masterPNRId).ToList();
        ...
        private InvoiceVendorDetailModel GetVendorBranchDetail(int agentid)
        {
            var agentDetail = _ent.Agents.Where(x => x.AgentId == agentid).FirstOrDefault();
            var branchDetail = _ent.BranchOffices.Where(x => x.BranchOfficeId == agentDetail.BranchOfficeId).FirstOrDefault();
            ...
            string zonename = branchDetail.Zones != null ? branchDetail.Zones.ZoneName : string.Empty;
            agencyDetail.VendorName = branchDetail.BranchOfficeName;
            agencyDetail.Address = zonename + " " + GetCountryById(branchDetail.NativeCountryId).CountryName;
            agencyDetail.PhoneNo = branchDetail.Phone;
            agencyDetail.PanNo = branchDetail.PanNo;
        ...
        private InvoiceAgencyDetailModel GetVendorDistributorDetail(int agentid)
        {
            var agentDetail = _ent.Agents.Where(x => x.AgentId == agentid).FirstOrDefault();
            ...
            agencyDetail.AgencyName = distributorDetail.DistributorName;
        ...
        {
            var PNR = _ent.PNRs.Where(x => x.AgentId == agentId);
            var filteredPNRS = PNR.Where(id => id.PNRId == masterPNRID).ToList();
