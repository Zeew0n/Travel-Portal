...
namespace ATLTravelPortal.Areas.Administrator.Controllers
{
    ...
    public class DistributorAgentManagementController : Controller
    {
        ...
        //[Authorize]
        public ActionResult Edit(int? id, string redirected)
        {
            _model = _rep.Detail(id, out _res);
            TempData["InfoMessage"] = _res.ActionMessage;
            if (_res.ErrNumber == 0)
            {
                ...
                _model.RedirectedFrom = redirected;
                return View(_model);
            }
            else
                return RedirectToAction("Index");
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (_res.ErrNumber == 0)
                    {
                        ...
                        {
                            return RedirectToAction("AgentList", "DistributorManagement", new { id = _model.DistributorId });
                        }
                        else if (model.RedirectedFrom == "BranchDistributorManagement")
                        {
                            return RedirectToAction("AgentList", "BranchDistributorManagement", new { id = _model.DistributorId });
                        }
                        else
                        {
                            return RedirectToAction("Index");
        ...
        public ActionResult AgentSearch(AgentModel model)
        {
            ...
            {
                var agentsearchlist = _rep.GetAgentSearchResultbydistributorid(model.AgentSearch.TrimStart(' ').TrimEnd(' '), obj.LoginTypeId);
