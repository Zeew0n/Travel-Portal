...
<div class="contentGrid">
    ...
    <%if (Model.CreditLimitList != null && Model.CreditLimitList.Count() > 0)
      { %>
    <strong>Details Credit Limit For
        <%:Model.CreditLimitList.FirstOrDefault().AgencyName %>(<%:Model.CreditLimitList.FirstOrDefault().AgencyCode %>)</strong>
    <table cellspacing="0" cellpadding="0" border="0" style="border-collapse: collapse;"
        ...
        <tbody>
            <tr>
                ...
                <td>
                    <%:(item.isApproved==true ? "Approved" :"Unapproved" )%>
                </td>
            </tr>
        </tbody>
        <%}
        ...
        <tbody>
            <tr style="background: #999;">
                <td colspan="6" style="text-align: left;">
                    <label>
                        Total: <strong>
                            <%:(string.Format(System.Globalization.CultureInfo.GetCultureInfo("hi-IN"), "{0:#,0.#}", totalamount))%></strong></label>
                </td>
            </tr>
        </tbody>
    </table>
