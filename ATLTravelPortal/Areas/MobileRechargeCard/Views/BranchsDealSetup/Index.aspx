    ...
    <script type="text/javascript">
        ...
        $('#DeleteMasterDeal').live("click", function (event) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/MobileRechargeCard/MasterBranchDealSetup/Delete/",
        ...
        function DeleteMasterDealForceFully(masterDealId, text) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/MobileRechargeCard/AjaxRequest/DeleteMobileBranchMasterDealForceFully/",
                    data: { id: masterDealId, name: text },
                    error: function () {
                        $("#loadingIndicator").html('');
                        alert('Can not delete the deal ' + text);
        ...
        function EditDeal(divId, dealId) {
            ...
            $('#' + divId).load("/MobileRechargeCard/AjaxRequest/AjaxBranchDealDetail/" + dealId );
        }

        function CancelDeal(divId, dealId) {
            $("#" + divId + "_loading").html('<center><img src="<%=Url.Content("~/Content/images/indicator.gif") %>" alt="" width="16px" height="16px" /></center>');
            $("#" + divId).load("/MobileRechargeCard/AjaxRequest/AjaxBranchDealCancel/" + dealId);
        }
        ...
        function DeleteDeal(divId, dealId) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/MobileRechargeCard/BranchsDealSetup/Delete/",
                    data: { id: dealId },
                    error: function () {
                        $("#" + divId + "_loading").html('');
        ...
        $('#saveDeal').live("click", function (event) {
            ...
            $.ajax({
                type: "POST",
                url: '/MobileRechargeCard/BranchsDealSetup/Create/',
        ...
        function UpdateDeal(divId, formId) {
            ...
            $.ajax({
                type: "POST",
                url: '/MobileRechargeCard/BranchsDealSetup/Edit/',
                data: serializeFormData,
                dataType: "html",
                success: function (serverResponse) {
        ...
        $(document).ready(function () {
            $("#DealMasterId").change(function () {
                ...
                if (id == "") {
                    ...
                    $.ajax({
                        type: "GET",
                        url: '/MobileRechargeCard/BranchsDealSetup/Index/',
