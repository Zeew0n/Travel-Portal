...
namespace ATLTravelPortal.Areas.MobileRechargeCard.Controllers
{
    ...
    public class DealSetupController : Controller
    {
        ...
        public ActionResult Index(int? id, string FilterDealIdentifierId)
        {
            var dealtype = mDealProvider.GetDealMasterById(id ?? 0);
            DealViewModel viewmodel = new DealViewModel();

            viewmodel.DealMasterList = mDealProvider.GetAllDealMasterList(3);
         

            viewmodel.CurrencyList = mDealProvider.GetCurrencyList();
            viewmodel.DealIdentifierList = mDealProvider.GetMobileDealIdentifiers(dealtype != null ? dealtype.DealTypeId : 0);
            viewmodel.DealList = mDealProvider.GetAllMobileDeals(id, FilterDealIdentifierId);
        ...
        public ActionResult Create(int id, int? FilterDealIdentifierId, string FilterDealIdentifierText)
        {
            ...
            viewmodel.DealIdentifierList = mDealProvider.GetMobileDealIdentifiers(mDealProvider.GetDealMasterById(id).DealTypeId);
        ...
        public ActionResult Create(DealViewModel model)
        {
            ...
            int dealId = mDealProvider.Save_Mobile_Deals(model);

            DealViewModel viewModel = new DealViewModel();

            viewModel = mDealProvider.GetMobileDealDetail(dealId);
        ...
        public ActionResult Edit(DealViewModel model)
        {
            ...
            DealViewModel viewModel = new DealViewModel();
            viewModel = mDealProvider.GetMobileDealDetail(model.DealId);
        ...
        public ActionResult Delete(int id)
        {
            ...
            try
            {
                bool result = false;
                TravelSession obj = (TravelSession)Session["TravelPortalSessionInfo"];
                result = mDealProvider.Delete_Mobile_Deals(id, obj.AppUserId);
