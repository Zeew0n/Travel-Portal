...
namespace ATLTravelPortal.Areas.Bus.Controllers
{
    ...
    public class BranchIssuedTicketController : Controller
    {
        ...
        public ActionResult Index(int? page, BusPNRModel _model)
        {
            ...
            try
            {
                ...
                _model.TabularList = _rep.GetBranchPagedIssueList(page, _model.FromDate, _model.ToDate, _model.AgentId, ts.LoginTypeId);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult Index(BusPNRModel model, int? page, ExportModel Expmodel, FormCollection frm)
        {
            ...
            try
            {
                var ts = (TravelSession)Session["TravelPortalSessionInfo"];
                ViewData["DisList"] = new SelectList(defaultProvider.GetDistributorList(ts.LoginTypeId), "DistributorId", "DistributorName");
                _model.TabularList = _rep.GetBranchPagedIssueList(page, model.FromDate, model.ToDate, model.AgentId, ts.LoginTypeId);
            }
            catch (Exception ex)
            {
                _model.Message = BusGeneralRepository.CatchException(ex);
