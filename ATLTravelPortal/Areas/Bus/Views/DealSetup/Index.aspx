<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/BusMain.Master"
    Inherits="System.Web.Mvc.ViewPage<ATLTravelPortal.Areas.Airline.Models.DealViewModel>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <div class="pageTitle">
        <ul class="buttons-panel">
            ...
            <li>
                <%:Html.ActionLink("New Deal", "Create","MasterDealSetUp", null, new { @class = "createDeal linkButton", @title = "New Deal" })%>
            </li>
            <li><a href="javascript:void(0);" id="DeleteMasterDeal" class="linkButton" title="Delete Deal">
                Delete Deal</a></li>
        </ul>
        <h3>
            <a href="#" class="icon_plane">Setup</a> <span>&nbsp;</span><strong>Deal SetUp</strong>
        </h3>
    </div>
...
<asp:Content ID="Content4" ContentPlaceHolderID="JSContent" runat="server">
    <script type="text/javascript">
        ...
        $('#DeleteMasterDeal').live("click", function (event) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Bus/MasterDealSetup/Delete/",
        ...
        function DeleteMasterDealForceFully(masterDealId, text) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Airline/AjaxRequest/DeleteBusMasterDealForceFully/",
                    data: { id: masterDealId, name: text },
                    error: function () {
                        $("#loadingIndicator").html('');
                        alert('Can not delete the deal ' + text);
        ...
        function EditDeal(divId, dealId) {
            ...
            $('#' + divId).load("/Bus/AjaxRequest/AjaxDealDetail/" + dealId + "?source=" + source);
        }

        function CancelDeal(divId, dealId) {
            $("#" + divId + "_loading").html('<center><img src="<%=Url.Content("~/Content/images/indicator.gif") %>" alt="" width="16px" height="16px" /></center>');
            $("#" + divId).load("/Bus/AjaxRequest/AjaxDealCancel/" + dealId);
        }
        ...
        function loadCreateDialog(tag, event, target) {
            ...
            url = url + '/' + id + '/?FilterDealIdentifierId=' + FilterDealIdentifierId + '&FilterDealIdentifierText=' + FilterDealIdentifierText + '&FilterBusOperatorId=' + FilterBusOperatorId + '&FilterBusCategoryId=' + FilterBusCategoryId + '&FilterCurrencyId=' + FilterCurrencyId;
        ...
        $(document).ready(function () {
            $("#DealMasterId").change(function () {
                ...
                if (id == "") {
                    ...
                    var url = "/Airline/AjaxRequest/GetBusDealIdentifiers";
                    ...
                    $.ajax({
                        type: "GET",
                        url: '/Bus/DealSetup/Index/',
                        data: { id: id, FilterDealIdentifierId: FilterDealIdentifierId, FilterBusOperatorId: FilterBusOperatorId, FilterBusCategoryId: FilterBusCategoryId, FilterCurrencyId: FilterCurrencyId },
                        dataType: "html",
                        async: true,
                        cache: false,
                        success: function (serverResponse) {
            ...
            $("#FilterDealIdentifierId").change(function () {
                ...
                if (id == "") {
                    ...
                    $.ajax({
                        type: "GET",
                        url: '/Bus/DealSetup/Index/',
                        data: { id: id, FilterDealIdentifierId: FilterDealIdentifierText, FilterBusOperatorId: FilterBusOperatorId, FilterBusCategoryId: FilterBusCategoryId, FilterCurrencyId: FilterCurrencyId },
                        dataType: "html",
                        async: true,
                        cache: false,
                        success: function (serverResponse) {
    ...
    <script type="text/javascript" language="javascript">
        $(document).ready(function () {
            $(".fromCityAutoComplete").live("change", function () {
                var text = $(this).val();
                var idCtrl = $(this).attr("id");
                var idCounter = idCtrl.substring(idCtrl.lastIndexOf('_') + 1);

                if (text == '')
                    $("#editFromCityId_" + idCounter).val('');
            });

            $(".toCityAutoComplete").live("change", function () {
                var text = $(this).val();
                var idCtrl = $(this).attr("id");
                var idCounter = idCtrl.substring(idCtrl.lastIndexOf('_') + 1);
                if (text == '')
                    $("#editToCityId_" + idCounter).val('');
            });
        });
    </script>
</asp:Content>
