    ...
    <script type="text/javascript">
        ...
        $('#DeleteMasterDeal').live("click", function (event) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Bus/MasterDistributorDealSetup/Delete/",
        ...
        function DeleteMasterDealForceFully(masterDealId, text) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Airline/AjaxRequest/DeleteBusDistributorMasterDealForceFully/",
                    data: { id: masterDealId, name: text },
                    error: function () {
                        $("#loadingIndicator").html('');
                        alert('Can not delete the deal ' + text);
        ...
        function EditDeal(divId, dealId) {
            ...
            source = $("#Source").val();
            $('#' + divId).load("/Bus/AjaxRequest/AjaxDistributorDealDetail/" + dealId + "?source=" + source);
        }

        function CancelDeal(divId, dealId) {
            $("#" + divId + "_loading").html('<center><img src="<%=Url.Content("~/Content/images/indicator.gif") %>" alt="" width="16px" height="16px" /></center>');
            $("#" + divId).load("/Bus/AjaxRequest/AjaxDistributorDealCancel/" + dealId);
        }
        ...
        function DeleteDeal(divId, dealId) {
            ...
            if (confirm(alertMsg)) {
                $.ajax({
                    type: "POST",
                    url: "/Bus/DistributorDealSetup/Delete/",
                    data: { id: dealId },
                    error: function () {
                        $("#" + divId + "_loading").html('');
        ...
        $('#saveDeal').live("click", function (event) {
            ...
            if (FromCity == '' && ToCity == '') {
                ...
                $.ajax({
                    type: "POST",
                    url: '/Bus/DistributorDealSetup/Create/',
        ...
        function UpdateDeal(divId, formId) {
            ...
            $.ajax({
                type: "POST",
                url: '/Bus/DistributorDealSetup/Edit/',
                data: serializeFormData,
                dataType: "html",
                success: function (serverResponse) {
        ...
        $(document).ready(function () {
            $("#DealMasterId").change(function () {
                ...
                if (id == "") {
                    ...
                    $.ajax({
                        type: "GET",
                        url: '/Bus/DistributorDealSetup/Index/',
