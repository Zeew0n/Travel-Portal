...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="pageTitle">
        ...
        <h3>
            <a href="#" class="icon_plane">Setup</a> <span>&nbsp;</span><strong> Edit Bus Schedule</strong>
        </h3>
    </div>
    <div class="row-1">
        <div class="form-box1 round-corner">
            <div class="form-box1-row">
                <div class="form-box1-row-content float-left">
                    <div> <%: Html.HiddenFor(model => model.ScheduleId)%>
            ...
            <div class="form-box1-row">
                <div class="form-box1-row-content ">
                    <div class="days">
                        <div><%: Html.LabelFor(model => model.Sunday)%>
                        <%: Html.CheckBoxFor(model => model.Sunday)%></div>
                        <div><%: Html.LabelFor(model => model.Monday)%>
                        <%: Html.CheckBoxFor(model => model.Monday)%></div>
                        <div><%: Html.LabelFor(model => model.Tuesday)%>
                        <%: Html.CheckBoxFor(model => model.Tuesday)%></div>
                        <div><%: Html.LabelFor(model => model.Wednesday)%>
                        <%: Html.CheckBoxFor(model => model.Wednesday)%></div>
                        <div><%: Html.LabelFor(model => model.Thursday)%>
                        <%: Html.CheckBoxFor(model => model.Thursday)%></div>
                        <div><%: Html.LabelFor(model => model.Friday)%>
                        <%: Html.CheckBoxFor(model => model.Friday)%></div>
                        <div><%: Html.LabelFor(model => model.Saturday)%>
         ...
         $(document).ready(function () {
             ...
             $("#BusMasterId").change(function () {
                 ...
                 if (id == "") {
                     ...
                     var url = "/BusSchedule/GetBusNumber";

                     $.getJSON(url, { MasterId: id }, function (data) {
                         ...
                         $.each(data, function (index, optionData) {
                             if (index == "Data") {
                                 ...
                                 $.each(optionData, function (key, value) {
                                     //alert(value['BusId']+"="+value['BusNumber']);

                                     $("#BusId").append("<option value='" + value['BusId'] + "'>" + value['BusNumber'] + "</option>");
                                 });
                             }
                         });
                     });
