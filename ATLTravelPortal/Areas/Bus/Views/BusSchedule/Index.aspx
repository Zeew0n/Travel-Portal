...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <div class="pageTitle">
        ...
        <h3>
            Bus <span>&nbsp;</span> Schedule Manager
        </h3>
    </div>
    <div id="messageBox">
    ...
    <div class="rptSearchResult">
        ...
        <table cellspacing="0" cellpadding="0" border="0" style="border-collapse: collapse;"
            ...
            <tbody>
                ...
                <tr id="tr_<%=sno %>" class="<%:classTblRow %>" onmouseover="this.className='GridRowOver'"
                    ...
                    <td>
                        <p>
                            <%:Html.ActionLink("Detail", "Details", new { id = item.ScheduleId, controller = "BusSchedule", area = "Bus" }, new { @class = "", @title = "Details" })%>
                            |
                            <%:Html.ActionLink("Edit", "Edit", new { id = item.ScheduleId, controller = "BusSchedule", area = "Bus" }, new { @class = "", @title = "Edit" })%>
                            |
                            <%:Html.ActionLink("Delete", "Delete", new { id = item.ScheduleId, controller = "BusSchedule", area = "Bus" }, new { @class = "", @title = "Delete", @onclick = "return confirm('Do you want to Delete (" + item.BusCategoryName + ")');" })%>
                        </p>
                    </td>
                </tr>
                <%  }%>
       ...
       <%-- <%  if (Model.TabularList.TotalItemCount > ATLTravelPortal.Areas.Bus.Repository.BusGeneralRepository.DefaultPageSize)
            {%>--%>
        <div class="pager">
            <%:MvcHtmlString.Create(ATLTravelPortal.Areas.Bus.Pagination.PagingExtensions.Pager(Html, ATLTravelPortal.Areas.Bus.Repository.BusGeneralRepository.DefaultPageSize, Model.TabularList.PageNumber, Model.TabularList.TotalItemCount, true, true, "", Url.Content("~/Bus/BusSchedule/Index")))%>
        </div>
        <%  }
              //}
              else
